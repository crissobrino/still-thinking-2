...
def run_main_evaluation():
    with open("evaluation/gold_standard2.json", "r") as f:
    ...
    for q, gold_ids in gold_standard.items():
        ...
        configs = [
            ...
            ("ENN (Pure Gold)", True, False)
        ]
        
        for name, pow, rerank in configs:
            resp = requests.post(API_URL, json={"query": q, "use_powerful_model": pow, "use_reranker": rerank, "eval_mode": (name=="ENN (Pure Gold)")}).json()
